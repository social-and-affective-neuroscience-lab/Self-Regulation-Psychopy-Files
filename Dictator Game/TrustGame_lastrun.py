...
CueLoop = data.TrialHandler(nReps=1, method='sequential', 
...
for thisCueLoop in CueLoop:
    ...
    # set up handler to look after randomisation of conditions etc
    randomlyPresent = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('spreadsheet5.xlsx', selection=Rows),
        seed=None, name='randomlyPresent')
    thisExp.addLoop(randomlyPresent)  # add the loop to the experiment
    thisRandomlyPresent = randomlyPresent.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisRandomlyPresent.rgb)
    if thisRandomlyPresent != None:
        for paramName in thisRandomlyPresent:
            exec('{} = thisRandomlyPresent[paramName]'.format(paramName))
    
    for thisRandomlyPresent in randomlyPresent:
        currentLoop = randomlyPresent
        # abbreviate parameter names if possible (e.g. rgb = thisRandomlyPresent.rgb)
        if thisRandomlyPresent != None:
            for paramName in thisRandomlyPresent:
                exec('{} = thisRandomlyPresent[paramName]'.format(paramName))
        ...
        for thisSelfishGiver in SelfishGiver:
            ...
            for thisComponent in isiComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            SelfishGiver.addData('isiText.started', isiText.tStartRefresh)
            SelfishGiver.addData('isiText.stopped', isiText.tStopRefresh)
        ...
        for thisComponent in isiComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        randomlyPresent.addData('isiText.started', isiText.tStartRefresh)
        randomlyPresent.addData('isiText.stopped', isiText.tStopRefresh)
        ...
        for thisGenerousGiver in GenerousGiver:
            ...
            for thisComponent in isiComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            GenerousGiver.addData('isiText.started', isiText.tStartRefresh)
            GenerousGiver.addData('isiText.stopped', isiText.tStopRefresh)
    ...
    # completed 1 repeats of 'randomlyPresent'
