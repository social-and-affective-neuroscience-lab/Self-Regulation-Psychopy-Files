...
thisExp = data.ExperimentHandler(name=expName, version='', extraInfo=expInfo, runtimeInfo=None,
    originPath='C:\\Users\\Jojo\\Downloads\\Helion Lab\\Experiment Files\\Food Regulation\\FoodRegulation.py',
...
CueClock = core.Clock()
cue = visual.TextStim(win=win, text='default',font='Arial', pos=(0, 0), height=0.13, color='white');
ISI = visual.TextStim(win=win, text='default',font='Arial', pos=(0, 0), height=0.1, color='white');

PicClock = core.Clock()
Image = visual.ImageStim(win=win, image='sin',pos=(0, 0), size=1.0, texRes=128, interpolate=True)

ratingsClock = core.Clock()
ratingQ = visual.TextStim(win=win, text='default',font='Arial', pos=(0, 0.15), height=0.07, wrapWidth=1.5, color='white');
rating = visual.RatingScale(win=win,marker='triangle', size=1.4, pos=[0.0, -0.35], low=1, high=7, labels=['Not at all', 'Very much'], scale='')
rating.respKeys = ['1', '2', '3', '4', '5', '6', '7' ]
isiClock = core.Clock()
...
practiceLoop = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('foodRows.xlsx', selection='0'),
...
BeginInstrComponents = [text, spacebar]
...
trials_2 = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('foodRows.xlsx', selection='0:5'),
